...
#else
    // pinMode(GPIO_NUM_16, OUTPUT);auto
    // pinMode(GPIO_NUM_17, INPUT); BLE_BAUD_RATE
    int bt_rate = 0;//sj_bt_rate->get();
    Uart0.setPins(1, 3);  // Tx 1, Rx 3 - standard hardware pins
    Uart0.begin(BAUD_RATE, Uart::Data::Bits8, Uart::Stop::Bits1, Uart::Parity::None);
    Uart2.setPins(GPIO_NUM_16,GPIO_NUM_17);  // Tx 1, Rx 3 - standard hardware pins
    Uart2.begin(BAUD_RATE, Uart::Data::Bits8, Uart::Stop::Bits1, Uart::Parity::None);
    // Uart2.begin((bt_rate>0?BAUD_RATE:BLE_BAUD_RATE), Uart::Data::Bits8, Uart::Stop::Bits1, Uart::Parity::None);
...
static uint8_t getClientChar(uint8_t* data) {
    ...
    if (client_buffer[CLIENT_BTSSP].availableforwrite() &&(res = Uart2.read()) != -1) { 
        *data = res;
        return CLIENT_BTSSP;
    }
...
#ifdef ENABLE_BLEGATT
    if (BLEGATTServer::getInstance().hasClient()) {
        if ((res = BLEGATTServer::getInstance().read()) != -1) {
            *data = res;
            return CLIENT_BTGATT;
        }
    }
#endif
...
void execute_realtime_command(Cmd command, uint8_t client) {
    switch (command) {
        ...
        case Cmd::StatusReport:
            SJCutterIns.cheakLedConnectStatus(client);  //检查LED灯状态
...
void client_write(uint8_t client, const char* text) {
...
#endif
#ifdef ENABLE_BLEGATT
    if (BLEGATTServer::getInstance().hasClient() && (client == CLIENT_BTGATT || client == CLIENT_ALL)) {
        BLEGATTServer::getInstance().send(text);
    }
#endif
    ...
    if (client == CLIENT_BTSSP || client == CLIENT_ALL) { 
        Uart2.write(text);    
    }
}

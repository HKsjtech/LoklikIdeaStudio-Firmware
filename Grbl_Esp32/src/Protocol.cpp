            ...
            while ((c = client_read(client)) != -1) {
                ...
                switch (res) {
                    ...
                    case Error::Eol:
                        ...
                        {
                            if (!strcmp(client_lines[client].buffer, "$SN")) {
                                // 设置序列号
                                SJCutterIns.readSerialNumber(client); 
                            }else if (!strcmp(client_lines[client].buffer, "$FV")) {
                                // 读取固件版本
                                SJCutterIns.readFirmwareVersion(client);
                            }
                        }
...
#endif
                        proc_status=SJCutterIns.handleMessage(client,line);
                        if(proc_status==CMDResult::RESULT_OK){
                            // 处理成功
                            report_status_message(Error::Ok, client); 
                        }else if(proc_status==CMDResult::RESULT_FAIL){
                            // 处理失败
                            report_status_message(Error::InvalidValue, client);                        
                        }else if(proc_status==CMDResult::RESULT_IGNORE){
                        ...
                        }else if(proc_status==CMDResult::RESULT_ABANDON){
                            // 抛弃消息
                        }
                        empty_line(client);                     
                        break;

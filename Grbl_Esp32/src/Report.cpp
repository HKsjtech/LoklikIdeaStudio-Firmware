...
void report_feedback_message(Message message) {  // ok to send to all clients
...
#endif  //ENABLE_SD_CARD
    {
        ...
        if (it != MessageText.end()) {
            grbl_msg_sendf(CLIENT_ALL, MsgLevel::Info, it->second);
        }
    }
}
...
#ifdef REPORT_SJCUTTER_STATUS
    char status_info_pkg[128];
    memset(status_info_pkg,0,128);
    // 获取传感器     
    SJCutterIns.readStatus((char*)status_info_pkg,0);
    sprintf(temp, "|%s", status_info_pkg);
    strcat(status, temp);
#endif
